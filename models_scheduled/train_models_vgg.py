...
parser = OptionParser()
parser.add_option("--bagging", dest="bagging", help="Run bagged model.", type = int, default = 1)
parser.add_option("--bagging_params", dest="bagging_params", help="Specify parameters for bagging.", type = int, default = 1)
parser.add_option("--skf", dest="skf", help="Run SKF model.", type = int, default = 1)
parser.add_option("--skf_params", dest="skf_params", help="Specify parameters for SKF run.", type = int, default = 1)
...
def parametrized_bagging(params):
    ...
    if params['run_training'] == True:
        bagged_model_run(model_savename, params['num_bags'], model_name, 200, batch_size = None, split = params['split_data'], prepare_sub = params['prepare_submission'], full_dst = params['train_src'], train_dst = 'train_set_gpu0', valid_dst = 'valid_set_gpu0', test_src = params['test_src'])
...
params_bagging1 = {
    ...
    'train_src': 'train_crops_vgg_299_oversampled',
    'test_src': 'test_crops_vgg_299',
    'run_name': 'avg1',
    'num_bags': 5,
    'run_training': True,
